...
/**
 * 看不懂的地方 1、checkIntensity() 2、getRootPath()
 */
...
/**
 * 自己写的 当密码改变时执行 checkConformPassword()
 */
function testChange(password){
	conform_password = $("#conform_password").val();
	if (password.value != conform_password) {
...
function checkUserName(obj) {
	if (checks(obj.value) == false) {
		...
		$.ajax({
			type : "POST",
			url : getRootPath() + "rest/register/checkName",
			dataType : 'json',
			data : 'userName_1=' + obj.value,
...
function checkEmail(email) {
	...
	if (email.value.length < 1) {
		...
		$.ajax({
			type : "post",
			url : getRootPath() + "rest/register/checkEmail",
			dataType : 'json',
			data : 'email_1=' + email.value,
			success : function(json) {
				if (json.flag) {
